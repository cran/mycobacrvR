iedb_consensus_mhcii_nmer = function (clas) 
{
  i1 = 1
  ...
  for (i in i1:i2) {
    ...
    class = as.matrix("iedb_consensus_mhcii", ncol = 1, 
                      byrow = T)
    ...
    write.table(out, file = paste("iedb_consensus_mhcii_", 
                                  strsplit(temp[rv], split = "epitopes", fixed = T)[[1]][2], 
                                  ".txt", sep = ""), sep = "\t", row.names = F)
    write.table(metadata, file = paste("iedb_consensus_mhcii_", 
                                       strsplit(temp[rv], split = "epitopes", fixed = T)[[1]][2], 
