abcpred_nmer = function (epi_length) 
{
  ...
  for (i in i1:i2) {
    ...
    temp6 = cbind(gi_number, checka, orthologs, epitope_length, 
                  epitope_seq, conservation_ratio, class, temp6)
    colnames(temp6)[1:7] = c("ginumber", "score", "Number_of_Orthologs", 
    ...
    write.table(out, file = paste("abcpred_", epi_length, 
                                  "_", strsplit(temp[rv], split = "epitopes", fixed = T)[[1]][2], 
                                  ".txt", sep = ""), sep = "\t", row.names = F)
    write.table(temp5, file = paste("abcpred_", epi_length, 
                                    "_", strsplit(temp[rv], split = "epitopes", fixed = T)[[1]][2], 
