iedb_consensus_mhci_nmer = function (epi_length) 
{
  i1 = 1
  ...
  for (i in i1:i2) {
    ...
    write.table(out, file = paste("IEDBconsensus_mhci_", 
                                  epi_length, "_", strsplit(temp[rv], split = "epitopes", 
                                                            fixed = T)[[1]][2], ".txt", sep = ""), sep = "\t", 
                row.names = F)
    write.table(metadata, file = paste("IEDBconsensus_mhci_", 
                                       epi_length, "_", strsplit(temp[rv], split = "epitopes", 
                                                                 fixed = T)[[1]][2], "_metadata.txt", sep = ""), 
                sep = "\t", row.names = F)
